...
function App() {
  const { density, focusMode } = useApp();

  return (
    <div className={`min-h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 selection:bg-zinc-900 selection:text-white transition-colors duration-300`}>
      <div className="flex flex-col md:flex-row max-w-[1600px] mx-auto min-h-screen">
        ...
        <main className={`flex-1 px-6 md:px-12 py-12 ${density === 'compact' ? 'space-y-12' : 'space-y-32'} transition-all`}>
          <Section title="Skills" id="skills">
            <Suspense fallback={<div className="h-64 flex items-center justify-center">Loading Skills Graph...</div>}>
              <SkillsGraph />
            </Suspense>
          </Section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {cvData.projects.map((proj, i) => (
                <motion.div 
                  whileHover={{ y: -5 }}
                  key={i} 
                  className="p-8 border-2 border-zinc-900 dark:border-zinc-100 group transition-colors hover:bg-zinc-900 hover:text-white dark:hover:bg-zinc-100 dark:hover:text-zinc-900"
                >
      ...
      {focusMode && (
        <button 
          ...
          className="fixed bottom-8 right-8 px-6 py-2 border-2 border-black dark:border-white font-bold uppercase text-xs"
        >
          Exit Focus Mode
        </button>
      )}
